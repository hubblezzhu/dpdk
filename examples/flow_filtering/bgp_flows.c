...
struct rte_flow *
generate_bgp_flow_src(uint16_t port_id, uint16_t rx_q,
		...
		struct rte_flow_error *error)
{
	...
	tcp_spec.hdr.src_port = htons(179);
	tcp_mask.hdr.src_port = htons(0xFFFF);
...
struct rte_flow *
generate_bgp_flow_dst(uint16_t port_id, uint16_t rx_q,
		uint32_t src_ip, uint32_t src_mask,
		uint32_t dest_ip, uint32_t dest_mask,
		struct rte_flow_error *error);
		...
		struct rte_flow_error *error)
{
	...
	memset(&tcp_spec, 0, sizeof(struct rte_flow_item_tcp));
	memset(&tcp_mask, 0, sizeof(struct rte_flow_item_tcp));
	tcp_spec.hdr.dst_port = htons(179);
	tcp_mask.hdr.dst_port = htons(0xFFFF);
	pattern[2].type = RTE_FLOW_ITEM_TYPE_TCP;
	pattern[2].spec = &tcp_spec;
	pattern[2].mask = &tcp_mask;
	/* >8 End of setting the third level of the pattern. */

...
#define VXLAN_MAX_PATTERN_NUM		4
#define VXLAN_MAX_ACTION_NUM		2

struct rte_flow *
generate_vxlan_flow(uint16_t port_id, uint16_t rx_q,
		...
		struct rte_flow_error *error)
{
	...
	struct rte_flow_item pattern[VXLAN_MAX_PATTERN_NUM];
	struct rte_flow_action action[VXLAN_MAX_ACTION_NUM];
	...
	/* Setting the third level of the pattern (UDP with VXLAN port). 8< */
	pattern[2].type = RTE_FLOW_ITEM_TYPE_UDP;
	/* >8 End of setting the third level of the pattern. */

	/* Setting the fourth level of the pattern (VXLAN). 8< */
	pattern[3].type = RTE_FLOW_ITEM_TYPE_VXLAN;
	/* >8 End of setting the fourth level of the pattern. */
